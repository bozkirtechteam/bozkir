...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $query = $db->prepare("SELECT * FROM admin WHERE kullanici_adi = ?");
    $query->execute([$kullanici_adi]);
    $admin = $query->fetch(PDO::FETCH_ASSOC);

    if ($admin && $sifre == $admin["sifre"]) { // Şifre hashli değilse bu şekilde kontrol edilir
        ...
        <?php else: ?>
            ...
            <form action="" method="POST">
                ...
                <div class="input-field">
                    <input type="text" name="kullanici_adi" id="email" required>
                    <label for="text">Enter your E-mail</label>
                </div>
                <div class="input-field">
